        ...
        if (nCode >= 0)
        {
            ...
            switch ((MouseMessages)wParam)
            {
                case MouseMessages.WM_LBUTTONDOWN:
                    MouseEvents.Add(new MouseEvent("LeftClick", mouseInfo.pt.x-XOffSet, mouseInfo.pt.y-YOffSet, timestamp));
                    break;
                case MouseMessages.WM_RBUTTONDOWN:
                    MouseEvents.Add(new MouseEvent("RightClick", mouseInfo.pt.x-XOffSet, mouseInfo.pt.y-YOffSet, timestamp));
                    break;
                case MouseMessages.WM_MOUSEMOVE:
                    //  MouseEvents.Add(new MouseEvent("Move", mouseInfo.pt.x, mouseInfo.pt.y, timestamp));
                    break;
                case MouseMessages.WM_MOUSEWHEEL:
                    // Extrahiere das Mausrad-Drehen (high-order word des mouseData)
                    int wheelDelta = (short)((mouseInfo.mouseData >> 16) & 0xFFFF);
                    MouseEvents.Add(new MouseEvent("Wheel",mouseInfo.pt.x-XOffSet, mouseInfo.pt.y-YOffSet, timestamp, wheelDelta));
                    break;
            }
        }
